...
#R means that the nucleotide is coding and part of a gene is reverse direction from right to left

#ann - CNR
#genomo - AGCT
def translate_ann_to_indices(ann, genome):
    i = 0
    length = len(ann)
    hidden_state_indices = []
    while i < length:
        if ann[i] == 'N':
            hidden_state_indices.append(0)
            if i+3 < length:
                if ann[i+1:i+4] == 'CCC': #NCCC
                    if genome[i+1:i+4] == 'ATG':
                        hidden_state_indices.extend([1,2,3])
                    elif genome[i + 1:i + 4] == 'GTG':
                        hidden_state_indices.extend([4, 5, 6])
                    elif genome[i + 1:i + 4] == 'TTG':
                        hidden_state_indices.extend([7, 8, 9])
                    else:
                        hidden_state_indices.extend([10, 11, 12])
                    i += 4
                elif ann[i+1:i+4] == 'RRR': #NRRR
                    if genome[i + 1:i + 4] == 'TTA':
                        hidden_state_indices.extend([22, 23, 24])
                    elif genome[i + 1:i + 4] == 'CTA':
                        hidden_state_indices.extend([25, 26, 27])
                    elif genome[i + 1:i + 4] == 'TCA':
                        hidden_state_indices.extend([28, 29, 30])
                    else:
                        hidden_state_indices.extend([31, 32, 33])
                    i += 4
                else:
                    i += 1
            else:
            ...
            if i+3 < length and ann[i+3] == 'N': #CCCN
                if genome[i:i+3] == 'TAG':
                    hidden_state_indices.extend([13,14,15])
                elif genome[i:i + 3] == 'TGA':
                    hidden_state_indices.extend([16, 17, 18])
                elif genome[i:i + 3] == 'TAA':
                    hidden_state_indices.extend([19, 20, 21])
                else:
                    hidden_state_indices.extend([0,0,0])
            else:
                hidden_state_indices.extend([10, 11, 12])
            i += 3
        elif ann[i] == 'R':
            if i+3 < length and ann[i+3] == 'N': #RRRN
                if genome[i:i+3] =='CAT':
                    hidden_state_indices.extend([34, 35, 36])
                elif genome[i:i + 3] == 'CAC':
                    hidden_state_indices.extend([37, 38, 39])
                elif genome[i:i + 3] == 'CAA':
                    hidden_state_indices.extend([40, 41, 42])
                else:
                    hidden_state_indices.extend([0, 0, 0])
            else:
                hidden_state_indices.extend([31, 32, 33])
            i += 3
    return hidden_state_indices
